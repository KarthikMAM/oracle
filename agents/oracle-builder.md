...
5. **Write the verdict.** Write `result.json` (build status, full-suite test pass/fail counts, modified-line coverage % in integration, and the log paths) and `verdict.json` to the mode's location:
